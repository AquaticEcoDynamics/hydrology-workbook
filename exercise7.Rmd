...
-	Nowadays you know all about [WIR](http://wir.water.wa.gov.au/Pages/Water-Information-Reporting.aspx), 
which is where you could get flow and water quality data. Track down the flow gauges on this map. *NOTE : THIS SITE IS DOWN @ 15 SEP 2025*
<br>
- Checkout the [Swan-Canning dashboard](https://scevodash-sce.azurewebsites.net). This was made by some UWA students, and is super useful for exploring data related to hydrology, water quality and other stuff. Give it some time to load and then :
a) Hit the "Water Quality" section. In the "Catchment" tab, navigate to find the Site called YULEB. Zoom into the highlighted site on the map, and check the "Subcatchments" box. Select the key nutrient variables of interest (e.g., TN and TP), and plot how they look over the last 5 years. What about over a 20 year period?
b) Search for another catchment on the map that has different looking land-cover (e.g. Bennett Brook), and now add that to the site list. Which one has higher TN or TP? Can you think why that might be?
c) If you change the variable to TON (which is nitrate), or PO4, how do things look? 
d) OK, now change to the "Profiles: Live" tab. This section takes raw profile data from a water quality sonde that is dropped at different sites, and is contoured to show the spatial patterns. In this section, set the region to Canning, and then choose a year (e.g. 2023) and a month. Then click plot, and wait. You can zoom in on the map to Canning River while you wait and check the "profile" item on the map. Now you can see the water quality parameters, like salinity, oxygen, and chlorophyll-a through a "slice" of the river. Try to idnetify where the Yule Brook intercepts the river and enters.
...
In addition, we also have explored some local water quality data, relavent to Yule Brook. 
